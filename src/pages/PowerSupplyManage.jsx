...
import { deletePower, putPower } from "../services/powerSupply";
import Classes from "../styles/ComponentManage.module.css";

function PowerSupplyManage() {
  const location = useLocation();
  const navigate = useNavigate();
  const [vendor, setVendor] = useState(location.state.Vendor);
  const [model, setModel] = useState(location.state.Model);
  const [powerInW, setPowerInW] = useState(location.state.PowerInW);
  ...
  async function saveChange(e) {
    ...
    if (window.confirm("Confirm Change!")) {
      putPower(location.state.id, data)
        .then((res) => {
...
export default PowerSupplyManage;
